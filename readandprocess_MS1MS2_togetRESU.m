...
%%
[data_afterMS1] = read_pretreat_match_MS1;
%%
data_afterMS1MS2 = [];
used_posi = [];
coll_temp = [];
...
for i_new = 3 : length(thefiles)
    ...
    toopen = ['fidin = fopen(' '''' filepath_MS2 '\' filename '''' ');'  ];
    eval(toopen);
    fidout1 = fopen('thedata.txt', 'w'); 
    fidout2 = fopen('thechar.txt', 'w'); 
    num = 1;
    while ~feof(fidin) 
        tline = fgetl(fidin); 
    ...
    for j_new = 1 : length(thestart)
        ...
        %% Addad -20190814_________________________________________________________
        ...
        if length(posi12) >= 1
            ...
            for k_new = thestart(j_new) + 4 : theend(j_new) - 1
              ...
              %% Addad-20190814
                ...
                coll_temp = [coll_temp; [data_afterMS1(posi12, :) temp str2num(cell2mat(regexp(filename,'\d', 'match')))]];%20190110ÐÞ¸Ä
            end
...
%% ADDED -20141226
[resu_refine_data_afterMS1MS2_final] = refine_data_afterMS1MS2_final(tol_tR, tol_mz);
...
X = csvread('resu_refine_data_afterMS1MS2_final.csv');
result_data = table(X(:,4), X(:,10), X(:,11), X(:,12), X(:,13),  'VariableNames', result_columns_name);
